...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, att_type=None):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, use_cbam=att_type == "CBAM")
            )
...
def ResidualNet(network_type, depth, num_classes, att_type):
    ...
    if depth == 18:
        model = ResNet(
            BasicBlock, [2, 2, 2, 2], network_type, num_classes, att_type
        )

    elif depth == 34:
        model = ResNet(
            BasicBlock, [3, 4, 6, 3], network_type, num_classes, att_type
        )

    elif depth == 50:
        model = ResNet(
            Bottleneck, [3, 4, 6, 3], network_type, num_classes, att_type
        )

    elif depth == 101:
        model = ResNet(
            Bottleneck, [3, 4, 23, 3], network_type, num_classes, att_type
        )
